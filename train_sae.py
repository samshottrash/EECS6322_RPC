...
def train_autoencoder(model, dataloader, num_epochs = 200, lr=1e-4):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 10 == 0:
            ...
            # save model checkpoint for future loading
...
if __name__ == "__main__":
    ...
    # Load the HF dataset from disk.
    embeddings = load_from_disk("cc3m_clip_features_test")
    print(f"Loaded HF dataset with {len(embeddings)} examples.")

    # Extract the clip_features column as a list (or np.array).
    ...
    SAE_trained_model = train_autoencoder(sae_model, dataloader)
